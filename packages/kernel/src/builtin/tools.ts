...
const editTool: Tool = {
  ...
  async execute(input, ctx: ToolContext) {
    ...
    await ctx.ports.fileSystem.writeFile(file_path, next);
...
const grepTool: Tool = {
  ...
  async execute(input, ctx: ToolContext) {
    ...
    const files = await ctx.ports.fileSystem.glob(glob ?? "**/*");
    const hits: string[] = [];
    for (const f of files) {
      let content: string;
      try {
        content = await ctx.ports.fileSystem.readFile(f);
      } catch {
        continue;
      }
...
const taskTool: Tool = {
  ...
  async execute(input, ctx: ToolContext) {
    ...
    try {
      const handle = await ctx.ports.multiAgent.spawn({ name: agentName, prompt: description });
